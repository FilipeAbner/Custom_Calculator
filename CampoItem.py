...
class CampoItem:
    def __init__(self, parent, remove_callback=None, pontuacao_fixa=None, titulo=None, quantidade=0):
        ...
        self.score_entry = ttk.Entry(self.frame, width=8, textvariable=self.score_var, justify="center")
        ...
        self.qty_entry = ttk.Entry(self.frame, width=8, textvariable=self.qty_var, justify="center")
        self.qty_entry.grid(row=0, column=3, padx=5)
        self.qty_entry.bind("<FocusIn>", self._clear_qty)

        if remove_callback:
            ttk.Button(self.frame, text="Remover", command=self._remove).grid(row=0, column=4, padx=5)
